...
from transformers import T5EncoderModel, T5Tokenizer, AutoConfig
...
from fastchat.train.modeling_rag import RagForCausalLM
...
def train_rag():
    ...
    # Initialize encoder
    encoder = T5EncoderModel.from_pretrained(model_args.question_encoder_name_or_path,
        trust_remote_code=True,
        cache_dir=training_args.cache_dir,
    )
    encoder.config.use_cache = False
    encoder_tokenizer = T5Tokenizer.from_pretrained(
        model_args.question_encoder_name_or_path,
        cache_dir=training_args.cache_dir,
        model_max_length=training_args.question_encoder_max_length,
        padding_side="right",
        use_fast=False,
    )
    # tokenizer.pad_token = tokenizer.unk_token
