...
def get_model_answers(model_path, model_id, question_jsons, question_encoder_path, retriever):
    ...
    '''
    encoder_dict = torch.load(model_path + '/pytorch_model.bin')
    # encoder = T5EncoderModel(T5Config())
    encoder = AutoModelForSequenceClassification.from_pretrained("textattack/bert-base-uncased-yelp-polarity", num_labels=1, ignore_mismatched_sizes=True)
    state_dict = {}
    ...
    encoder_tokenizer = AutoTokenizer.from_pretrained("textattack/bert-base-uncased-yelp-polarity", num_labels=1, ignore_mismatched_sizes=True)
    '''
